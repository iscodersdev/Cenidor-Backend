...
{
    [Area("Core")]
    public class BannersController : CenidorCoreController
    {
        public BannersController(CenidorContext context) : base(context)
        {
        ...
        [HttpPost]
        public async System.Threading.Tasks.Task<ActionResult> Create(Banners banner, int colorId, int BannersFijo)
        {
            try
            {
                var usuario = _context.Usuarios.FirstOrDefault(x => x.Email == User.Identity.Name);
        ...
        [HttpPost]
        public async System.Threading.Tasks.Task<ActionResult> Update(Banners banner, int colorId, int BannerFijo)
        {
