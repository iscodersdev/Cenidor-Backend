...
namespace CenidorCore.API.Controllers.Billetera
{
    ...
    public class MTarjetasController : BaseApiController
    {
        ...
        public MTarjetasController(CenidorContext context) : base(context)
        {
        }
        ...
        public IActionResult SolicitarPagoTarjeta([FromBody] PagoTarjetaDTO pagotarjetaDTO)
        {
            try
            {
                ...
                //var listaMovimiento = movtarj.ConsultarMovimientosTarjetas(empresa.UsernameWS, empresa.PasswordWS, usuario.Personas.NroDocumento, pagotarjetaDTO.NroTarjeta, 1,0);
                //var nuevosMovimientos = movtarj.ConsultarMovimientosTarjetas2(empresa.UsernameWS, empresa.PasswordWS, usuario.Personas.NroDocumento, Convert.ToUInt32(pagotarjetaDTO.NroTarjeta), 100, movimientostarjetaDTOS.tipomovimiento);
                ...
                //if (listaMovimiento.Resultado == "EXITO")
               // {
                    //var resultado2 = listaMovimiento.DetalleMovimientosTarjeta.Where(y => y.Fecha <= Convert.ToDateTime(listaMovimiento.FechaPagoProximaCuota)).ToList();////;
                    //var resu = resultado2.Sum(x => Convert.ToDecimal(x.Monto.Replace(",", "."))).ToString().Replace(".", ",");
                    //var resu = resultado2.Sum(x => Convert.ToDecimal(x.Monto.Replace(".",",").ToString())).ToString();
