...
namespace CenidorCore.Controllers
{
    ...
    public class MImagenIntroController : BaseController
    {
        ...
        public MImagenIntroController(CenidorContext context, UserService<Usuario> userManager)
        {
            _context = context;
        ...
        public MTraeImagenIntroDTO TraeImagenIntro([FromBody] MTraeImagenIntroDTO uat)
        {
            ...
            var iamgenes = _context.ImagenIntro.Where(x => x.Orden != 0 && x.Foto!=null)
                .OrderBy(x => x.Orden).Take(4).Select(x => new MImagenIntro { Fecha = x.Fecha, Id = x.Id, Titulo = x.Titulo, Orden = x.Orden, Imagen = Convert.FromBase64String(x.Foto) }).ToList();
            if (iamgenes.Count > 0)
            {
