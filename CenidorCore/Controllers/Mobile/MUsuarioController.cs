...
namespace CenidorCore.Controllers
{
    ...
    public class MUsuarioController : BaseController
    {
        ...
        public MUsuarioController(CenidorContext context, UserService<Usuario> userService, SignInManager<Usuario> signInManager, ICompositeViewEngine viewEngine, IServiceProvider serviceProvider)
        {
            _context = context;
        ...
        public async Task<MLoginDTO> Login([FromBody] MLoginDTO Login)
        {
            ...
            if (autorizado)
            {
                ...
                Login.Status = 200;
                Login.UAT = common.Encrypt(DateTime.Now.ToString("ffffssmmHHddMMyyyy") + cliente.Id.ToString(), "Cenidor");
        ...
        public async Task<MRecuperaPasswordDTO> RecuperaPassword([FromBody] MRecuperaPasswordDTO uat)
        {
            ...
            //string pass = common.Encrypt(cliente.Persona.NroDocumento.ToString() + DateTime.Now.ToString(), "Cenidor");
            ...
            {
                if (user.Personas==null && cliente.Persona!=null) {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (Registro.FormularioRegistro == 5) // Valida con la tarjeta
                    {
                        ...
                        var cliente = _context.Personas.Where(x => x.NroTarjeta == result).FirstOrDefault();
                        if (cliente!=null)
                        {
                            preregistro.Status = 500;
                            preregistro.Mensaje = "El Nro de Tarjeta le Pertenece a un Socio ya registrado!!";
        ...
        public async Task<MRegistraPersonaDTO> RegistraPersona20([FromBody] MRegistraPersonaDTO Registro) //Utilizado por la App Mobile
        {
            ...
            var personalocal = _context.Personas.FirstOrDefault(x => x.Email == Registro.Mail.ToString());
            var clienteLocal = _context.Clientes.FirstOrDefault(x => x.Persona.Email == Registro.Mail.ToString());
            int token = common.NiumeroRandom(100000, 999999);
                ...
                if (clienteLocal.Persona!=null) // Si la persona Existe
                {
                    if (user!=null)
                    {
                        ...
                        else
                        {
                            ...
                            CreateOrUpdateUser(user, clienteLocal, Registro.Mail, token, Registro.Password1);
            ...
            else
            {
                ...
                Persona personaLocal = _context.Personas.Where(x => x.Email==Registro.Mail).FirstOrDefault();

                if (personaLocal!=null)
                {
                    cliente.Persona = personaLocal;
                    cliente.RegistroMobile = true;
                    user = CreateOrUpdateUser(user, cliente, Registro.Mail, token, Registro.Password1);
                    var result = await _userService.ChangePasswordAsync(user, "xahs567g", Registro.Password1);
                }
                else
                {
