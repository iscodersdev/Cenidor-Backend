...
namespace CenidorCore.Controllers
{
    ...
    public class MBannersController : BaseController
    {
        ...
        public MBannersController(CenidorContext context, UserService<Usuario> userManager)
        {
            _context = context;
        ...
        public MTraeBannersDTO TraeBanners([FromBody] MTraeBannersDTO uat)
        {
            ...
            var banner = _context.Banners.Where(x => x.Empresa.Id == Uat.Cliente.Empresa.Id && (uat.UltimaId == 0 || x.Id < uat.UltimaId) && x.Foto != null).OrderBy(x => x.Orden).Select(x => new MBanners { BannerFijo= x.BannerFijo, Fecha = x.Fecha, Texto = x.Texto, Id = x.Id, Titulo = x.Titulo, Subtitulo=x.Subtitulo, Link = x.Link, Imagen = Convert.FromBase64String(x.Foto), Orden= x.Orden }).ToList();
            if (banner.Count > 0)
            {
                uat.Banners = banner;
            }
        ...
        public MTraeCabeceraBannersDTO TraeCabecera([FromBody] MTraeCabeceraBannersDTO uat)
        {
            try
            {
                ...
                var banner = _context.Banners.Where(x => (uat.UltimaId == 0 || x.Id < uat.UltimaId) && x.Foto != null).OrderBy(x => x.Orden).Take(2).Select(x => 
