...
namespace CenidorCore.Controllers
{
    ...
    public class MPromocionesController : BaseController
    {
        ...
        public CenidorContext _context;
        public MPromocionesController(CenidorContext context, UserService<Usuario> userManager)
        {
            _context = context;
        ...
        public MTraePromocionesDTO TraePromociones([FromBody] MTraePromocionesDTO uat)
        {
            ...
            //var promociones = _context.Promociones.Where(x => x.Empresa.Id == Uat.Cliente.Empresa.Id && (uat.UltimaId == 0 || x.Id < uat.UltimaId) && x.Foto != null && (x.FechaHasta.Date.CompareTo(DateTime.Now.Date)) >= 0).OrderBy(x => x.Orden).Select(x => new MPromociones { PromocionFija= x.PromocionFija, Fecha = x.Fecha, Texto = x.Texto, Id = x.Id, Titulo = x.Titulo, Subtitulo=x.Subtitulo, Link = x.Link, QR = x.QR, Imagen = Convert.FromBase64String(x.Foto) }).ToList();
            //var promociones = _context.Promociones.Where(x => x.Empresa.Id == Uat.Cliente.Empresa.Id && (uat.UltimaId == 0 || x.Id < uat.UltimaId) && x.Foto != null).OrderBy(x => x.Orden)
            //    .Select(x => new MPromociones { PromocionFija= x.PromocionFija, Fecha = x.Fecha, Texto = x.Texto, Id = x.Id, Titulo = x.Titulo, Subtitulo=x.Subtitulo, Link = x.Link, QR = x.QR, Imagen = Convert.FromBase64String(x.Foto) }).ToList();
            ...
            var promocionesVencimiento = _context.Promociones.Where(x => x.Vencimiento==true && (x.FechaHasta.Date.CompareTo(DateTime.Now.Date)) >= 0).OrderBy(x => x.Orden)
                .Select(x => new MPromociones { PromocionFija= x.PromocionFija, Fecha = x.Fecha, Texto = x.Texto, Id = x.Id, Titulo = x.Titulo, Subtitulo=x.Subtitulo, Link = x.Link, QR = x.QR, Imagen = Convert.FromBase64String(x.Foto), Orden = x.Orden }).ToList();
        ...
        public MTraeCabeceraPromoDTO TraeCabecera([FromBody] MTraeCabeceraPromoDTO uat)
        {
            try
            {
                ...
                var promocionesVencimiento = _context.Promociones.Where(x => x.Vencimiento==true && (x.FechaHasta.Date.CompareTo(DateTime.Now.Date)) >= 0).OrderBy(x => x.Orden).Take(2).Select(x =>
                new MCabeceraPromo
                {
                    Id = x.Id,
                    Imagen = Convert.FromBase64String(x.Foto),
